...
$(function () {
        ...
        function submitAddStore() {
            $('#addStoreForm').on('submit', function () {
                if (!checkInputVal($('input[name=name]'), '活動名稱')) {
                    return false;
                }
            })
        }
        ...
        function submitGroupBuy() {
            $('#groupBuyForm').on('submit', function () {
                ...
                if (!group_host_new.val() && $('select[name=group_host]').val() === '0') {
                    checkInputVal(group_host_new, '活動負責人');  // Check input not empty
                    return false;
                }
            })
        }
        ...
        function deleteStore() {
            $('.btn-del-store').on('click', function (e) {
                ...
                let deleteHandle = function () {
                    const alertInfo = new SwalAlert('資料已刪除！', '刪除一筆活動資料', '', 'success');
                    deleteAct.get()
                    alertInfo.fire()
        ...
        function showOrder(res = '') {
            ...
            if(res === 'error') {
                ...
                const alert = new SwalAlert('操作成功', '已刪除一筆揪團單', '', 'success').fire();
            }
        ...
        /**
         *
         * @param groupBuy  進行中活動單
         * @param totalOrders  全部訂單
         * @param stores  活動數據
         * @param groupBuyHistory  歷史活動單
         */
        function groupBuyDisplay(groupBuy, totalOrders, stores, groupBuyHistory) {
            ...
            if (groupBuyData.length === 0) {
                $('#current_groupBuy').text('目前還沒有活動 :(').addClass('text-center');
            } else {
        ...
        function delGroupBuy() {
            ...
            const delGroupBuyVar = function () {
                ...
                // const alertInfo = new SwalAlert('操作成功', '一筆活動單已刪除', '', 'success');
                // alertInfo.fire();
            }
        ...
        function ordersListDisplay(ordersData) {
            ...
            $('#ordersNum').html(`共有 ${totalName.length} 人參與活動，累積有 <b>${ordersData.length}</b> 筆訂單`);
        ...
        `;
            ...
            getStoreDeliveryAmount(allTotal);  // 獲取揪團門檻是否達標
